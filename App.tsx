...
import ReceiverView from './components/ReceiverView'; // Import ReceiverView
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // V6 Hash Parsing Protocol (Nano Code)
    ...
    if (rawHash) {
       ...
       if (code) {
          ...
          // Note: We pass the full translations object so decoder can find the wish text
          ...
          if (decoded) {
              ...
              // 2. Set Language from decoded data (if present)
  ...
  useEffect(() => {
    if (initialData) {
        const scrollToTarget = () => {
            ...
            // Scroll to top instantly for receiver
